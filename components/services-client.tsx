...
import type { Service } from "@/lib/services-actions"
import {
  Calculator, FileText, Users, Building, TrendingUp,
  Shield, Briefcase, Globe, DollarSign, BarChart,
  BookOpen, Settings, Star, Zap, Lock, Heart,
} from "lucide-react"
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          {services.map((service) => (
            <div key={service.id} className="group h-64 sm:h-72 md:h-80" style={{ perspective: "1000px" }}>
              <div
                ...
                <div className="absolute inset-0 w-full h-full" style={{ backfaceVisibility: "hidden" }}>
                  <Card className="h-full w-full border-0 shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                    <CardHeader className="pb-3 px-4 sm:px-6">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 bg-sky-100 rounded-lg flex items-center justify-center mb-3">
                        <ServiceIcon name={service.icon_name} className="h-5 w-5 sm:h-6 sm:w-6 text-sky-600" />
                      </div>
                      ...
                      <ul className="space-y-1.5 sm:space-y-2">
                        {(service.features ?? []).slice(0, 4).map((feature, idx) => (
                          <li key={idx} className="text-xs sm:text-sm text-gray-600 flex items-center gap-2">
                            <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-sky-600 rounded-full flex-shrink-0" />
                  ...
                  <Card className="h-full w-full border-0 shadow-md bg-gradient-to-br from-sky-50 to-blue-50 flex flex-col justify-center items-center overflow-hidden">
                    <CardContent className="text-center space-y-3 sm:space-y-4 p-4 sm:p-6 md:p-8 flex flex-col justify-center h-full">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-sky-100 rounded-full flex items-center justify-center mb-2">
                        <ServiceIcon name={service.icon_name} className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 text-sky-600" />
                      </div>
